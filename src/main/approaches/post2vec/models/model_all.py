...
class MultiComp(nn.Module):
    ...
    def __init__(self, args):
        ...
        dc_Ks = args.desc_code_kernel_sizes
        if len(t_Ks) == len(dt_Ks) and len(dt_Ks) == len(dc_Ks):
            num_of_Ks = len(t_Ks)
        else:
        ...
        self.desc_code_embed = nn.Embedding(dc_V, D)
        ...
        self.convs_dc = nn.ModuleList([nn.Conv2d(Ci, Co, (K, D)) for K in dc_Ks])
        self.dropout = nn.Dropout(args.dropout)

        self.fc1 = nn.Linear(num_of_Ks * Co * 3, C)
